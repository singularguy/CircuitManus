...
# @Version: V8.1.1-WS-PJS-FIXED - Perfected JSON Schema Integration (Prompt Fix)
# @Author: Your Most Loyal, Dedicated, Meticulous & Now Extremely Cautious Programmer (Corrected Prompt Omission)
# @Date: [Current Date] - Corrected omission in response generation prompt.
# @License: MIT License
# @Description:
# ==============================================================================================
#  Manus 系统 V8.1.1-WS-PJS 技术实现说明 (完美JSON方案集成版 - Prompt修正)
# ==============================================================================================
#
# V8.1.1-WS-PJS 核心改动 (基于 V8.1.0-WS-PJS):
# 1.  **修正 `_get_response_generation_prompt_v8_1` 方法**:
#     - 移除了之前错误的占位注释 "【假设它包含了完整的结构描述】". 
#     - 完整地嵌入了适用于响应生成阶段的 V8.1 JSON 结构描述,确保 LLM 在此阶段也能接收到完整的格式指令. 
#       虽然与规划阶段的JSON结构大部分重叠,但为了LLM的明确理解和代码的自包含性,不再省略. 
#     - 对嵌入的JSON结构描述中的某些字段(如 `execution_phase` 和 `decision.IS_CALL_TOOLS`)的描述,
#       针对响应生成阶段的特定要求(例如,必须为 "response_generation" 和 false)进行了微调和强调. 
#
# (继承 V8.1.0-WS-PJS 的所有特性)
# ==============================================================================================
...
log_file_name = os.path.join(
    LOG_DIR,
    f"agent_log_v8_1_1_pjs_fixed_{current_time_for_log.strftime('%Y%m%d_%H%M%S')}_{current_time_for_log.microsecond // 1000:03d}_P{os.getpid()}.log"
)
...
class LLMInterface:
    ...
    async def call_llm(self, messages: List[Dict[str, Any]], execution_phase: str, status_callback: Optional[Callable[[Dict], Awaitable[None]]] = None) -> Any:
        ...
        logger.info(f"[LLMInterface] 准备异步调用 LLM ({self.model_name}, Phase: {execution_phase}, Expecting V8.1.1 JSON)...") # Version bump in log
...
# --- 模块化组件: OutputParserV8_1 (与V8.1.0一致) ---
class OutputParserV8_1:
    def __init__(self, agent_tools_registry: Optional[Dict[str, Dict[str, Any]]] = None): 
        logger.info("[OutputParserV8_1] 初始化输出解析器 (适配 ManusLLMResponse-V8.1-Perfected JSON结构). ")
    ...
    def parse_llm_response_to_structured_json(self, llm_api_response_message: Any, execution_phase: str) -> Tuple[Optional[Dict[str, Any]], str, List[Dict[str,str]]]: 
        parser_id = f"parse_v81_{str(uuid4())[:8]}"
        logger.debug(f"[{parser_id}-OutputParserV8_1] 开始解析 LLM 响应 (Phase: {execution_phase})...")
        ...
        if llm_api_response_message is None:
            ...
            logger.error(f"[{parser_id}-OutputParserV8_1] 解析失败: {error_message}")
        ...
        logger.debug(f"[{parser_id}-OutputParserV8_1] 接收到的原始 LLM content (完整):\n{raw_content}")
        ...
        if match_md_json:
            ...
            logger.debug(f"[{parser_id}-OutputParserV8_1] 从 Markdown 代码块中提取到 JSON 字符串. ")
        else:
        ...
        try:
            ...
            logger.error(f"[{parser_id}-OutputParserV8_1] 解析时未知错误: {error_message}", exc_info=True)
        ...
        if not isinstance(parsed_json_dict, dict):
            ...
            logger.error(f"[{parser_id}-OutputParserV8_1] 结构验证失败: {error_message}")
            return None, error_message, [{"json_path": "root", "issue_description": error_message}]
        ...
        if status_val == "failure":
            ...
            if not isinstance(error_details_obj, dict):
                ...
                if not isinstance(error_details_obj.get("error_type"), str) or not error_details_obj["error_type"].strip():
                ...
                if not isinstance(error_details_obj.get("error_code"), str) or not error_details_obj["error_code"].strip():
                ...
                if not isinstance(error_details_obj.get("technical_message"), str) or not error_details_obj["technical_message"].strip():
        ...
        if not isinstance(parsed_json_dict.get("thought_process"), str) or not parsed_json_dict.get("thought_process","").strip(): 
            logger.warning(f"[{parser_id}-OutputParserV8_1] 'thought_process' 字段为空或类型不正确,但仍会接受. LLM Prompt 应强调其重要性. ")
        ...
        if not isinstance(decision_obj, dict):
            ...
            if is_call_tools_val is True: 
                if not isinstance(tool_call_requests, list):
                    ...
                    logger.warning(f"[{parser_id}-OutputParserV8_1] 'IS_CALL_TOOLS' 为 true 但 'TOOL_CALL_REQUESTS' 列表为空. 这可能是一个规划逻辑问题. ")
        ...
        logger.info(f"[{parser_id}-OutputParserV8_1] LLM 响应 (Phase: {execution_phase}, LLM_ID: {parsed_json_dict.get('llm_interaction_id', 'N/A')}) 已成功解析并验证为 ManusLLMResponse-V8.1-Perfected JSON结构！")
        return parsed_json_dict, "", [] 


# --- 模块化组件: ToolExecutor (与V8.1.0一致, 但 agent_instance 类型改为 CircuitAgent) ---
class ToolExecutor:
    def __init__(self, agent_instance: 'CircuitAgent', max_tool_retries: int = 1, tool_retry_delay_seconds: float = 1.0): 
        logger.info("[ToolExecutor] 初始化工具执行器 (支持异步, 重试, 失败中止, UI回调增强 V8.1.1). ") # Version bump in log
    ...
    async def execute_tool_calls(self, tool_call_requests_from_plan: List[Dict[str, Any]], status_callback: Optional[Callable[[Dict], Awaitable[None]]] = None) -> List[Dict[str, Any]]:
        executor_id = f"exec_v811_{str(uuid4())[:8]}" # Version bump in id
        logger.info(f"[{executor_id}-ToolExecutor] 准备异步执行 {len(tool_call_requests_from_plan)} 个工具调用请求 (V8.1.1)...") # Version bump in log
        ...
        for i, tool_request in enumerate(tool_call_requests_from_plan):
            ...
            for retry_attempt in range(self.max_tool_retries + 1): 
                ...
                try:
                    ...
                    action_result_this_attempt = {"status": "failure", "message": f"错误: 执行工具 '{function_name}' 时发生内部错误. ", "error": {"error_type": "TOOL_EXECUTION_ERROR", "error_code": "UNEXPECTED_TOOL_EXECUTION_FAILURE", "technical_message": err_msg_exec, "exception_details": traceback.format_exc(limit=3)}}
                    if retry_attempt == self.max_tool_retries: 
                        action_result_final_for_tool = action_result_this_attempt
...
class CircuitAgent: # 类名更新
    def __init__(self, api_key: str, model_name: str = "glm-4-flash-250414", 
        ...
        if console_handler: 
            ...
            logger.info(f"[AgentV8_1_1_WS Init] 控制台日志级别已设置为: {logging.getLevelName(console_log_level)} (Verbose Mode: {self.verbose_mode})")
        else:
            logger.warning("[AgentV8_1_1_WS Init] Console handler not found,无法动态设置日志级别. ")

        self.tools_registry: Dict[str, Dict[str, Any]] = {}
        logger.info("[AgentV8_1_1_WS Init] 正在动态发现并注册工具...")
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if hasattr(method, '_is_tool') and method._is_tool:
                ...
                if schema and isinstance(schema, dict) and 'description' in schema and 'parameters' in schema:
                    ...
                    logger.info(f"[AgentV8_1_1_WS Init] ✓ 已注册工具: '{name}'")
                else:
                    logger.warning(f"[AgentV8_1_1_WS Init] 发现工具 '{name}' 但其 Schema 结构不完整或无效,已跳过注册. ")
        if not self.tools_registry:
            logger.warning("[AgentV8_1_1_WS Init] 未发现任何通过 @register_tool 注册的工具！Agent 将主要依赖直接问答. ")
        else:
            logger.info(f"[AgentV8_1_1_WS Init] 共发现并注册了 {len(self.tools_registry)} 个工具. ")
            if logger.isEnabledFor(logging.DEBUG):
                try: logger.debug(f"[AgentV8_1_1_WS Init] 工具注册表详情:\n{json.dumps(self.tools_registry, indent=2, ensure_ascii=False)}")
        ...
        try:
            ...
            self.output_parser = OutputParserV8_1(agent_tools_registry=self.tools_registry) 
            ...
            logger.critical(f"[AgentV8_1_1_WS Init] 核心模块初始化失败: {e}", exc_info=True)
            raise 
            
        self.planning_llm_retries = max(0, planning_llm_retries)
        self.max_replanning_attempts = max(0, max_replanning_attempts)
        
        logger.info(f"[AgentV8_1_1_WS Init] 规划LLM重试次数: {self.planning_llm_retries}, 工具执行重试次数: {max_tool_retries}, 最大重规划尝试次数: {self.max_replanning_attempts}")
        
        logger.info(f"\n{'='*30} CircuitAgent 初始化成功 {'='*30}\n")

    # --- Action Implementations (与V8.1.0一致) ---
    @register_tool(
    ...
    # --- Orchestration Layer Method (V8.1.1-PJS Core Logic) ---
    async def process_user_request(self, user_request: str, status_callback: Callable[[Dict[str, Any]], Awaitable[None]]) -> None:
        ...
        logger.info(f"\n{'='*25} CircuitAgent 开始处理用户请求 (ReqID: {self.current_request_id}) {'='*25}") # Version bump in log
        logger.info(f"[OrchestratorV8_1_1_WS] 收到用户指令: \"{user_request}\"") # Version bump in log

        try:
            if not user_request or user_request.isspace():
                logger.info(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 用户指令为空. ")
            ...
            try:
                ...
                logger.error(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 添加用户消息到短期记忆时出错: {e_mem_user}", exc_info=True)
                ...
                await status_callback({"type": "final_response", "request_id": self.current_request_id, "llm_interaction_id": mem_err_json["llm_interaction_id"], "content": mem_err_json["decision"]["RESPONSE_TO_USER"]["content"], "final_v8_1_json_if_success": None})
                return
            ...
            while replanning_loop_count <= self.max_replanning_attempts:
                ...
                log_prefix = f"[OrchestratorV8_1_1_WS - PlanAttempt {current_planning_attempt_num} - ReqID: {self.current_request_id}]" # Version bump in log
                logger.info(f"\n--- {log_prefix} 开始 ---")
                    ...
                    try:
                        ...
                        if parsed_plan_v8_1_this_llm_call and not parser_error_msg_this_llm_call and not parsed_failed_validation_points_this_llm_call:
                            if parsed_plan_v8_1_this_llm_call.get("status") == "success":
                                logger.info(f"{log_prefix} 成功解析并验证V8.1.1 JSON计划. LLM报告状态为 'success' (LLM_ID: {active_llm_interaction_id}). Agent采纳此计划. ") # Version bump in log
                                ...
                                logger.warning(f"{log_prefix} LLM报告的V8.1.1 JSON计划状态为 'failure': {error_detail_from_llm} (LLM_ID: {active_llm_interaction_id}). Agent将不采纳此计划,并尝试让LLM修正(如果还有LLM调用重试次数). ") # Version bump in log
                            ...
                            if parsed_plan_v8_1_this_llm_call and parsed_plan_v8_1_this_llm_call.get("status") == "failure": 
                                try: self.memory_manager.add_to_short_term(llm_msg_obj_planning.model_dump(exclude_unset=True))
                                 ...
                                 sim_err_plan_content = { 
                                    ...
                                    "timestamp_utc": datetime.now(timezone.utc).isoformat(), "status": "failure", 
                                    "error_details": {"error_type": "LLM_OUTPUT_VALIDATION_ERROR", "error_code": "V8_1_JSON_VALIDATION_FAILED", "technical_message": parser_error_msg_this_llm_call, "is_direct_llm_failure": False, "failed_validation_points": parsed_failed_validation_points_this_llm_call },
                ...
                if not agent_accepted_latest_plan_for_action: 
                    error_summary_final_planning_llm_attempt = parser_error_msg_this_llm_call or "在多次LLM调用尝试后,未能从LLM获取可接受的V8.1.1规划JSON. " # Version bump in log
                    if parsed_failed_validation_points_this_llm_call: 
                    ...
                    if replanning_loop_count >= self.max_replanning_attempts:
                        ...
                        sim_fail_plan_content_for_replan = {
                            ...
                            "timestamp_utc": datetime.now(timezone.utc).isoformat(), "status": "failure", 
                            "error_details": {"error_type": "LLM_OUTPUT_VALIDATION_ERROR", "error_code": "V8_1_JSON_VALIDATION_FAILED_IN_PLAN_ATTEMPT", "technical_message": error_summary_final_planning_llm_attempt, "is_direct_llm_failure": False, "failed_validation_points": parsed_failed_validation_points_this_llm_call },
                ...
                if should_call_tools_v8_1:
                    ...
                    logger.info(f"{log_prefix} 决策: 直接回复 (V8.1.1 JSON). 无需工具调用. ") # Version bump in log
                    if isinstance(response_user_obj_v8_1_plan, dict) and response_user_obj_v8_1_plan.get("content","").strip():
                        tool_execution_results_for_llm_history = [] 
                        final_llm_v8_1_json_output_for_user_facing_reply = current_llm_plan_v8_1_obj 
                        logger.info(f"{log_prefix} 规划阶段决定直接回复,内容有效. 将使用此V8.1.1 JSON作为最终输出. LLM_ID: {final_llm_v8_1_json_output_for_user_facing_reply.get('llm_interaction_id')}") # Version bump in log
                        break 
                    else: 
            ...
            if not agent_accepted_latest_plan_for_action and replanning_loop_count > self.max_replanning_attempts:
                logger.error(f"[OrchestratorV8_1_1_WS - FinalPrep - ReqID:{self.current_request_id}] 已达最大重规划次数,且最终规划尝试仍失败. 将使用上次记录的错误信息. ") # Version bump in log
                final_llm_v8_1_json_output_for_user_facing_reply = None 
               ...
               final_llm_v8_1_json_output_for_user_facing_reply.get("decision",{}).get("IS_CALL_TOOLS") is True: 
                logger.info(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 工具执行成功,开始生成最终响应...") # Version bump in log
                ...
                try:
                    ...
                    if parsed_final_v8_1_resp_json and not final_parser_err_resp and not final_validation_failures_resp and parsed_final_v8_1_resp_json.get("status") == "success":
                        ...
                        logger.info(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 成功解析并验证最终响应V8.1.1 JSON (LLM_ID: {final_llm_interaction_id_for_user}). ") # Version bump in log
                        ...
                        err_msg_final_resp_gen = final_parser_err_resp or "V8.1.1最终响应JSON校验失败. " # Version bump in log
                        ...
                        logger.error(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] LLM未能生成有效V8.1.1 JSON最终回复: {err_msg_final_resp_gen}") # Version bump in log
                    ...
                    logger.critical(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] LLM最终响应调用或处理失败: {e_llm_final_gen_call}", exc_info=True) # Version bump in log
                ...
                logger.info(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 使用规划阶段的直接回复V8.1.1 JSON作为最终输出. ") # Version bump in log
                final_llm_interaction_id_for_user = final_llm_v8_1_json_output_for_user_facing_reply.get("llm_interaction_id")
                 ...
                 logger.error(f"[OrchestratorV8_1_1_WS - ReqID:{self.current_request_id}] 流程结束时,final_llm_v8_1_json_output_for_user_facing_reply 为空,表明处理失败. 将使用之前记录的错误信息. ") # Version bump in log
            ...
            await status_callback({"type": "final_response", "request_id": self.current_request_id, "llm_interaction_id": final_llm_interaction_id_for_user, "content": final_reply_for_user.strip() if final_reply_for_user else "抱歉,未能生成有效的回复. ", "final_v8_1_json_if_success": final_llm_v8_1_json_output_for_user_facing_reply }) 
            ...
            if not (final_llm_v8_1_json_output_for_user_facing_reply and final_llm_v8_1_json_output_for_user_facing_reply.get("status") == "success"):
                final_assistant_synthetic_error_message_v8_1 = {
                    "request_id": self.current_request_id,
                ...
                try: self.memory_manager.add_to_short_term({"role": "assistant", "content": json.dumps(final_assistant_synthetic_error_message_v8_1, ensure_ascii=False)})
    ...
    # --- Helper Methods for Prompts (V8.1.1-PJS Versions) ---
...
```
""" # End of v8_1_json_schema_description_for_prompt
        
        direct_qa_example_v8_1 = ( 
            "\n【通用示例1: 直接回答用户问题 (无需工具) - V8.1.1 JSON】\n" # Version bump in example title
            "如果用户问: “你好,什么是电容？”\n"
        ...
        if is_replanning: 
            replan_example_v8_1 = (
                "\n【重规划示例 (V8.1.1 JSON): 工具失败后,成功重规划并调用新/修正的工具】\n" # Version bump in example title
                "假设历史记录中有如下用户请求和失败的工具调用: \n"
                "  User: \"连接 R10 和 C5\"\n"
                "  Assistant (Previous Plan V8.1.1 JSON): ... (Planned connect_components_tool for R10, C5, llm_interaction_id: " + example_prev_tool_call_id + "_plan) ...\n"
                ...
                "您在【当前重规划】时,分析后发现R10不存在,需要先添加. 您的新V8.1.1 JSON输出应类似: \n"
                "```json\n"
                "{\n"
                ...
                "  \"error_details\": null,\n" 
                "  \"execution_phase\": \"planning\",\n"
                "  \"thought_process\": \"分析历史记录: 用户想连接R10和C5. 上一个计划 (llm_interaction_id: " + example_prev_tool_call_id + "_plan) 中调用connect_components_tool (tool_call_id: " + example_prev_tool_call_id + "_tool) 失败了,因为R10不存在. 因此,我的新计划是先添加R10. 我将假设C5已存在或用户会后续处理. 我将规划先用add_component_tool添加R10 (提供一个默认值如1kΩ,如果用户未指定),然后再用connect_components_tool连接R10和C5. 本次规划是成功的,因此status为success. \",\n"
                "  \"decision\": {\n"
                "    \"IS_CALL_TOOLS\": true,\n"
                "    \"TOOL_CALL_REQUESTS\": [\n"
                "      {\n"
                ...
                "    \"RESPONSE_TO_USER\": {\n"
                "      \"content_type\": \"text/plain\",\n"
                "      \"content\": \"看起来元件R10之前不存在. 我将先尝试添加一个1kΩ的电阻R10,然后再将它与C5连接. 如果C5也不存在,请之后告诉我C5的类型和值. \",\n"
                "      \"suggestions_for_next_steps\": [\n"
                "        {\"text_for_user\": \"确认C5是否存在并提供其信息. \"},\n"
        ...
        prompt_parts = [
            "您是一位顶尖的、极其严谨的电路设计编程助理 (Agent Version V8.1.1-WS-PJS). 您的行为必须专业、精确,并严格遵循指令. 您的输出【必须且只能是】一个符合下述V8.1.1规范的JSON对象,且该JSON对象需要被三个反引号和'json'标记包围 (即 ```json ... ```). JSON对象之外不应有任何其他文本、注释或解释. \n\n" # Version bump in text
            "【核心任务: 规划阶段 (V8.1.1)】\n" # Version bump in text
        ...
        prompt_parts.extend([
            ...
            f"当前电路与记忆摘要:\n{memory_context}\n\n" 
            "【最后再次强调】: 您的输出【必须且只能是】一个被 ```json ... ``` 包围的、严格符合上述V8.1.1规范的单个JSON对象. JSON对象之外不应有任何其他文本. 请务必仔细检查JSON的语法和所有字段的类型及条件要求！任何格式偏差都将导致处理失败！" # Version bump in text
        ])
    ...
    def _get_response_generation_prompt_v8_1(self, memory_context: str, tool_schemas_desc: str, request_id: Optional[str] = None) -> str: 
        ...
        # BOSS, 这里是关键修正！确保完整的V8.1 JSON结构描述被包含！
...
```
""" # End of v8_1_json_schema_description_for_resp_phase
        ...
        return (
            "您是一位顶尖的电路设计编程助理 (Agent Version V8.1.1-WS-PJS),经验丰富,技术精湛,并且极其擅长清晰、准确、诚实地汇报工作结果. 您的输出【必须且只能是】一个符合下述V8.1.1规范的JSON对象,且该JSON对象需要被三个反引号和'json'标记包围 (即 ```json ... ```). JSON对象之外不应有任何其他文本、注释或解释. \n\n" # Version bump in text
            "【核心任务: 响应生成阶段 (V8.1.1)】\n" # Version bump in text
            ...
            "【最后再次强调】: 您的输出【必须且只能是】一个被 ```json ... ``` 包围的、严格符合上述V8.1.1规范的单个JSON对象. 在这个阶段,您【绝对不能】再请求调用任何新工具. 您的任务是总结并回复. " # Version bump in text
        )
